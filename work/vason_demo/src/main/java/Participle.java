...
/**
 *
 * @describe    方法一： 二维数组存放（缺点：数据量大了占用空间）：
 *                      横轴和纵轴，横纵初始都为0，遍历横纵轴，标记出包含在字典里的数组，标记为1。因为要匹配，所以横轴的第一个位置必定有一个纵轴为1，纵轴最后一个值必定有一个横轴为1。
 *                      这样可以通过最后一个纵轴看是往前递归（下一个的纵轴为当前横轴减1）。直到匹配完纵轴（ 0-1 = -1 ）返回true，否则返回false
 *
 *              方法二： 将字符串与字典匹配，并用队列存放匹配后的起始位置，匹配一组后出队列，对空间消耗小：
 *                      循环字典dict,将字典与字符串s匹配，记录出现的位置，放入队列queue，然后移除，记录移除的位置， 当移除的位置等于 s.length时，表示全部匹配成功，返回true，否则返回false
 *
 *              下面代码采用方法二。
 * @author 陈启威
 * @date 2020-03-17
            ...
            if (prticiple[t] == 0){   //未匹配的位置进行匹配
                ...
                for (int i = 0; i < list.size(); i++) {  //循环判断字典，进行匹配
                    if ((temp.length() >= list.get(i).length())
                            && (temp.indexOf(list.get(i)) == 0)) {   //截取后的剩余字段大于等于字典中的长度，且匹配的首个位置是0，否则不匹配
                        queue.add(t + list.get(i).length());         //存放匹配后的起始位置
                    }
                }
            }
